...
export async function setCurrentCycleId(cycleId) {
  ...
  // Broadcast to clients so they can refresh scoped data and invalidate caches
  try {
    const channel = supabase.channel('settings-channel')
    await channel.send({
      type: 'broadcast',
      event: 'CURRENT_CYCLE_CHANGED',
      payload: { id: cycleId }
    })
  } catch (e) {
    console.warn('Warning: unable to broadcast current_cycle change', e)
  }
  return true
}
