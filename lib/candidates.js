...
import { getCurrentCycleId } from './settings'
...
export async function getCandidates() {
  try {
    ...
    if (currentCycleId) {
      query = query.eq('cycle_id', currentCycleId)
    }
...
export async function getCandidatesWithVoteStats() {
  try {
    ...
    if (currentCycleId) {
      candidatesQuery = candidatesQuery.eq('cycle_id', currentCycleId)
      // Ensure nested votes are restricted to the same cycle
      candidatesQuery = candidatesQuery.eq('votes.cycle_id', currentCycleId)
    }
    ...
    if (currentCycleId) {
      currentRoundQuery = currentRoundQuery.eq('cycle_id', currentCycleId)
    }

    const { data: currentRound } = await currentRoundQuery

    // 3) Gather vote aggregates depending on whether a round is open.
    let roundVotes = []
    if (currentRound) {
      let rvQuery = supabase
        .from('votes')
        .select('pnm_id, score')
        .eq('round_id', currentRound.id)
      if (currentCycleId) rvQuery = rvQuery.eq('cycle_id', currentCycleId)
      const { data: rv } = await rvQuery
      roundVotes = rv || []
    } else {
...
export async function getVoteStats(candidateId) {
  try {
    ...
    if (currentCycleId) votesQuery = votesQuery.eq('cycle_id', currentCycleId)
    ...
    if (currentCycleId) roundsQuery = roundsQuery.eq('cycle_id', currentCycleId)
    ...
    if (currentCycleId) allVotesQuery = allVotesQuery.eq('cycle_id', currentCycleId)
...
export async function getInteractionStats(candidateId, roundId = null) {
  try {
    ...
    if (currentCycleId) query = query.eq('cycle_id', currentCycleId)
