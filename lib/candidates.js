...
export async function getVoteStats(candidateId) {
  try {
    ...
    // Top-level Bayesian:
    //   – If there is an open round, use that round's Bayesian.
    //   – Otherwise, compute Bayesian across ALL rounds using overall stats.
    let bayesian = 0
    ...
    if (openRound) {
      // Use Bayesian of that round (already computed above)
      const openName = allRounds.find(r => r.id === openRound.id)?.name || ''
      const st = roundStats[openName]
      bayesian = st?.bayesian ?? 0
    } else {
      // No open round – compute Bayesian using overall candidate stats
      ...
      bayesian = denom === 0 ? 0 : numer / denom
    }
